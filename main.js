...
Main.main = function() {
	...
	Main.result = window.document.getElementById("result");
...
Main.search = function(query) {
	...
	var http = new haxe_http_HttpJs("https://ja.wikipedia.org/w/api.php?action=query&list=search&format=json&origin=*&srsearch=" + encodeURIComponent(query));
	http.onData = function(data) {
		...
		Main.result.innerHTML = "<h2>検索結果</h2>";
		var _g = 0;
		while(_g < searchResults.length) {
			var item = searchResults[_g];
			++_g;
			var rawSnippet = "";
			if(item != null && item.snippet != null) {
				rawSnippet = item.snippet;
			}
			var snippet = Main.cleanSnippet(rawSnippet);
			var title = "";
			if(item != null && item.title != null) {
				title = item.title;
			}
			Main.addResult(title,snippet);
		}
	};
	http.onError = function(e) {
		Main.result.innerHTML = "エラー: " + e;
	};
	http.request();
};
Main.cleanSnippet = function(snippet) {
	if(snippet == null) {
		return "";
	}
...
Main.addResult = function(title,snippet) {
	var container = window.document.createElement("div");
	var link = window.document.createElement("a");
	link.href = "https://ja.wikipedia.org/wiki/" + encodeURIComponent(title);
	link.target = "_blank";
	link.innerText = title;
	var desc = window.document.createElement("div");
	...
	s.padding = "8px";
	s.borderRadius = "6px";
	Main.result.appendChild(container);
};
Math.__name__ = true;
var Std = function() { };
Std.__name__ = true;
